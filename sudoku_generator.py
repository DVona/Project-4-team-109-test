...
class SudokuGenerator:
    ...
    def valid_in_row(self, row, num):
        cols_to_check = list(range(self.row_length))
    ...
    def valid_in_col(self, col, num):
        rows_to_check = list(range(self.row_length))
    ...
    def is_valid(self, row, col, num):
        ...
        if self.valid_in_row(row, num):
            if self.valid_in_col(col, num):
                return self.valid_in_box(box_row, box_col, num)
    ...
    def fill_box(self, row_start, col_start):
        unused_in_box = list(range(1, self.row_length + 1))
        for i in range(3):
            for j in range(3):
                self.board[row_start+i][col_start+j] = unused_in_box.pop(random.randint(0,len(unused_in_box)-1))
    ...
    def remove_cells(self):
        ...
        for i in range(len(self.board)):
            ...
            for j in range(len(self.board[i])):
                self.solved_board[i].append(self.board[i][j])
